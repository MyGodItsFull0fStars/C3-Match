...
def choice(nodes):
    resources = nodes
    # print(resources)
    resources_str = "["+(" ".join(str(x)
                         for x in resources)).replace(" ", ",") + "]"
    #print ((resources_str))
    ...
    # for task in ((tasks)):
    command = str.encode(os.popen(
        "python3.9 ranking_levelized.py "+tasks_str+" " + resources_str).read())
    ...
    try:
        ...
        if target.name != '':
            ...
            v1.create_namespaced_binding(
                namespace, body, _preload_content=False)
